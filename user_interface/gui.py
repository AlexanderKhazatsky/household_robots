...
class RobotGUI(tk.Tk):
    ...
    def __init__(self, robot=None):
        ...
        self.camera_order = np.arange(6)#TODO: robot.num_cameras()
        ...
        # Organize Frame Dict #
        self.frames = {}
        for F in (LoginPage, RobotResetPage, InstructionsPage, PreferedTasksPage,
                  SceneConfigurationPage, CameraPage, EnlargedImagePage, NoisePage,
                  RequestedBehaviorPage, SceneChangesPage, PreferedTasksPage):
    ...
    def set_img(self, i, widget=None, width=None, height=None):
        if self.time_index is None: img = get_images()[self.camera_order[i]]
        else: img = last_traj[self.time_index][self.camera_order[i]]
    ...
    #     while True:
    #         info = robot.get_info()
    #         # when we press a or b, map to space
    #         if a or b: self.event_generate('<KeyRelease-space>')
...
class SceneConfigurationPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        box_lbl = tk.Button(self, text =' ' * 25, highlightbackground='blue',
            font=Font(slant='italic', weight='bold'), padx=30, pady=39)
        box_lbl.place(relx=bx, rely=by, anchor='n')
...
class RequestedBehaviorPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Update Based Off Activity #
        controller.bind("<KeyRelease>", self.start_trajectory, add="+")
    ...
    def start_trajectory(self, event):
        if self.controller.curr_frame != self: return
        if event.keysym == 'space': self.controller.show_frame(CameraPage, wait=True)
...
class CameraPage(tk.Frame):
    def __init__(self, parent, controller, n_rows=2):
        super().__init__(parent)
        self.controller = controller
        self.n_cols = math.ceil(len(self.controller.camera_order) / n_rows)
        self.n_rows = n_rows
    ...
    def update_timer(self, seconds):
        ...
        self.controller.after(100, lambda: self.update_timer(seconds + 0.1))

    def end_trajectory(self):
        save = True #TODO: check if we kept or deleted the last traj

        # Update Based Off Success / Failure #
        if self.mode == 'practice_traj': self.controller.show_frame(SceneConfigurationPage)
        ...
        # Move To Next Page
        if scene_change: self.controller.show_frame(SceneChangesPage)
        else: self.controller.show_frame(RequestedBehaviorPage)
